...
const TaskComments = ({ taskId }: any) => {
  ...
  <div className={`border relative ${openReplies?.open ? "w-[60%]" : "w-[100%]"} `}>
    {/* Comments Header */}
    ...
    <div className="max-h-[450px] pb-[150px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-200">
      <div ref={commentsContainerRef} className="card-body pb-[5rem] px-4 flex flex-col">
        <div className="member-comments space-y-3 flex-1 pr-3">
          {groupedComments?.length > 0 ? (
            groupedComments.map((group: any, index: number) => {
              const filtersReplyComments = group.comments.filter((comment: any) => comment.replyTo === null);
              const formattedDate = momentWithTimezone(group.date, "MMMM D, YYYY");
                          ...
                          <div className="flex space-x-4 justify-end">
                            ...
                            <div className="member-profile-image">
                              <img
                                ...
                                src={comment.profilePictureUrl || "/profile-picture.png"}
                                onError={(e: any) => {
                              ...
                              <div className="member-name pr-10">
                                <span className="font-semibold">
                                  {IsUserCommentOrNot(comment)
                                    ? "You"
                                    : `${comment.firstName} ${comment.lastName || "Unknown"}`}
                                </span>
                              ...
                              <div className="person-message mt-2 text-slate-500 leading-snug pr-10">
                                {editingCommentId === comment.id ? (
                                  <div className="flex flex-col">
                                    <CKEditorComponent
                                      ...
                                      handleEditorChange={handleEditCommentChange}
                                    />
                                    <div className="mt-3 flex justify-end space-x-3">
                                      <button className="text-gray-500" onClick={handleCancelEdit}>
                                        Cancel
                                      </button>
                                      <button className="text-[#28A745]" onClick={handleSaveEdit}>
                                        Save
                                      </button>
                                    </div>
                                  </div>
                                ...
                                <p
                                  className={`text-[#3368a1] font-normal text-sm cursor-pointer ${
                                    getRepliesCount(group.comments, comment.id)[0] === 0 ? "hidden" : ""
                                  }`}
                                  onClick={() => handleReplyComment(comment)}
                                >
                                  {getRepliesCount(group.comments, comment.id)[1]}
                                </p>
                              </div>
                                  ...
                                  >
                                    <DropdownMenuItem
                                      className={`cursor-pointer ${
                                        IsUserCommentOrNot(comment) ? "" : "hidden"
                                      }`}
    ...
    {/* Footer Section */}
    <div className="card-footer border-t bg-[#EEEEF8] px-4 py-4 sticky bottom-0 right-0 left-0 z-50">
      <div className="grid grid-cols-[40px,auto] space-x-3">
        ...
        <div className="ck-editor-send-button grid grid-cols-[auto,50px] space-x-3 items-end">
          <div className="overflow-auto">
            {taskId && (
              <CKEditorComponent editorData={commentText} handleEditorChange={handleTestDetailsChange} />
            )}
          </div>
