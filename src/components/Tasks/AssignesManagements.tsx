...
const AssigneeManagment = () => {
  ...
  const confirmSelection = () => {
    ...
    const updatedMembers = [...selectedMembers, ...newMembers];
    const payload = updatedMembers.map((member: any) => ({
      user_id: member.id,
      role: member.role,
    }));
    ...
    mutate({ project_members: payload });
  };
  ...
  const changeRole = (userId: number, role: string) => {
    ...
    // Send payload as { project_members: [ { user_id, role } ] }
    mutate({ project_members: payload });
  };
  ...
  const { isFetching, isLoading } = useQuery({
    queryKey: ["getSingleMembers", taskId, updating],
    queryFn: async () => {
      if (!taskId) return;
      try {
  ...
  const { mutate: removeMembers } = useMutation({
    mutationFn: async (payload: any) => {
      setErrorMessages({});
      setLoading(true);
      return await deleteAssignesAPI(taskId, payload);
    },
    onSuccess: (response: any) => {
      if (response?.status === 200 || response?.status === 201) {
        toast.success(response?.data?.message);
        setSelectedMembers(
          selectedMembers.filter((member: any) => member.id !== taskId)
        );
        setUpdating((prev: any) => prev + 1);
...
export default AssigneeManagment;
